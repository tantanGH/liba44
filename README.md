...
実装後に ADPCMLIB 0.03の存在を教えて頂きました。確認したところ atop_make_buffer のバグは解消されており、さらに高速化されていましたが、API仕様及びエンコード・デコードのロジック自体は同じでしたので特に変更は入れていません。0.03のオリジナルのアーカイブは `ADPCMLIB003.LZH` としてこちらもツリーに含めてあります。
...
以下のAPIの扱うバイナリ構造については0.02とも0.03とも互換性はありません。
