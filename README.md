...
実装後に ADPCMLIB 0.03の存在を教えて頂きました。確認したところ atop_make_buffer のバグは解消されており、さらに高速化されていましたが、エンコード・デコードのロジック自体は同じでしたので特に変更は入れていません。0.03のオリジナルのアーカイブは `ADPCMLIB003.LZH` としてこちらもツリーに含めてあります。
